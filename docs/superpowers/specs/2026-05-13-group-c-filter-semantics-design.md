...
|---|---|---|---|
| Q1 | How is "conjunction non-empty" computed? | Real AND across `stack.topPost.relations` arrays | Each stack has a `relations` array where each entry carries its own `category`. Build `candidateSet = currentFilters ∪ {hoveredCat}`. A stack "covers" candidateSet if its relations contain every category in candidateSet. If any stack covers candidateSet → ADD; else → SWITCH. Implemented in `decideFilterMode`. |
...
| Q2 | Which posts count as matching AND filter? | A post matches if its `rel` category is in the active set (OR semantics for display); AND check uses `relations` array for conjunction decision | Display filtering uses OR: show stacks whose `rel` is in the active filter set. The conjunction ADD-vs-SWITCH decision uses the richer `relations` array so a stack can satisfy multi-category intersection. |
...
- **AND semantics.** The conjunction check uses `stack.topPost.relations` (multi-valued per stack), not the top-level `rel` string. `decideFilterMode(stacks, currentFilters, candidate)` builds `candidateSet = currentFilters ∪ {candidate}` and returns ADD if any stack's relations cover every category in candidateSet, SWITCH otherwise. This is true AND — if no stack has all required categories, we switch rather than accumulate a filter set that would show zero results.
- **Tag click and conjunction.** C4 (tag click) calls `handleFilterChipClick(cat)` which uses `decideFilterMode` against the full `relatedStacks` array (not the filtered display), so the AND check always has the complete data set available.

## 9. Judgment Calls Made Without User Input

| # | Ambiguity | Choice Made | Rationale |
|---|---|---|---|
| JC1 | "Conjunction non-empty" — stacks have multi-category data via `stack.topPost.relations` (an array where each relation carries its own `category`). | Real AND across relations arrays: build `candidateSet = currentFilters ∪ {X}`. For each stack, collect all distinct `category` values from `stack.topPost.relations`. If any stack's category set is a superset of `candidateSet`, the intersection is non-empty → ADD (new filter = `currentFilters ∪ {X}`). Otherwise → SWITCH (new filter = `{X}`). Implemented as `decideFilterMode(stacks, currentFilters, candidate)` in `RelatedStacks.tsx`. | Matches the user's stated intent: "AND is asking if there are any posts with existing filter plus the one being hovered/clicked." Example: active = {evidence, disagree}, hover X — if any stack's relations include both evidence and disagree and X, we ADD; otherwise SWITCH. |
